...
`;
  ...
  ...basicRelatedKeywords.slice(0, 10).map(k => ({
    ...
    popularity: Number(k.popularity) || 50,
    trend: (k.trend || 'stable') as 'rising' | 'stable' | 'declining',
  ...
  ...fallbackKeywords.map(kw => ({
    ...
    searchIntent: 'commercial' as const,
    difficulty: (Math.random() > 0.5 ? 'low' : 'medium') as 'low' | 'medium' | 'high'
  }))
].slice(0, 20) ,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('data : ' , finalAnalysis);
    return NextResponse.json({
      success: true,
      data: finalAnalysis,
      meta: {
